...
closed:
    select_screen();
    blit_copy_own(scene_ds_far(0x138F));                                 /* CAR gbo0 */
    return;

redraw:
    DSB(DS_gearbox_dirty) = 0;
    gfx_select_target(far_make(CODE_SEG, DSW(DS_buf_desc_c)));
    blit_copy_hot(scene_ds_far(0x1393), 0, 0);                           /* CAR gbox */
    {
        s16 x = (s16)(u16)(DSW(DS_knob_x) - DSW(DS_gbox_x));
        s16 y = (s16)(u16)(DSW(DS_knob_y) - DSW(DS_gbox_y));
        blit_and_clip_hot(scene_ds_far(0x139B), x, y);                   /* gnab */
        blit_or_clip_hot(scene_ds_far(0x1397), x, y);                    /* gnob, same stacked x,y */
    }
    select_screen();
...
void draw_dashboard_dynamic(void)
{
    ...
    if (DSB(DS_g_stageTime) & 8) {                                        /* DS:80A4 tick count */
        ...
        if (bx != 0) {
            ...
            if (!((s16)bx < (s16)DSW(DS_r_road_ptr))) {                   /* sub bx,[186F] ; jl */
                ...
                snd_play_oneshot(far_make(DGROUP, 0x0B0F));
                bx = (u16)((bx >> 2) & 0xFC);
                rad = scene_ds_far((u16)(0x13C3 + bx));                  /* rad5, rad4, ... */
                lit = true;
            }
        }
        if (!lit) rad = scene_ds_far(0x13BF);                             /* rad0 */
    } else {
        rad = scene_ds_far(0x13D7);                                       /* radb */
    }
    blit_copy_own(rad);
    ...
    /* ---- steering wheel pose (XOR toggles) */
    {
        ...
        if (bl != DSB(DS_wheel_state)) {
            ...
            FarPtr w = scene_ds_far((u16)(0x13DF + ((u16)bl << 1)));     /* whl1 / whl3 */
            select_screen();
            blit_xor_own(w);
        }
    }
    ...
    /* ---- wheel marker on a 90 px radius, with save-under */
    {
        ...
        if (!neg) dx = (u16)-dx;
        u16 ax = DSW(0x269D);                                               /* car wheel y */
        ax = (u16)((ax & 0xFF00) | (u8)((u8)ax - DSB((u16)(DS_wheel_cos90 + k))));
        dx = (u16)(dx + DSW(0x269B));                                       /* car wheel x */

        u16 sy = (u16)(ax - 2);
        u16 sx = (u16)((u16)(dx - 2) & 0xFFF8);
        ...
        blit_and_hot(scene_ds_far(0x138B), (s16)dx, (s16)ax);             /* dota */
        blit_or_hot(scene_ds_far(0x1387), (s16)dx, (s16)ax);              /* dot  */
    ...
    blit_copy_hot(scene_ds_far(0x13B7), 0, 0);                            /* inst */

    if (DSW(0x27FB) == 1) {                                                 /* needle gauges */
        ...
        FarPtr inl = scene_ds_far((u16)(0x139F + w));
        FarPtr ina = scene_ds_far((u16)(0x13A3 + w));
        blit_and_own(ina);
        blit_or_own(inl);
        ...
        blit_or_clip_own(scene_ds_far(0x140F));                           /* spdo bar */
